...
var T = {
  en: {
    ...
    faqItems: [
      { q: "What is the minimum deposit to start playing?", a: "You can fund your account with any amount starting from just $10, so there's no need to commit a big bankroll to get started." },
      { q: "What is the RTP (Return to Player) of CluckRun?", a: "CluckRun runs at a high 98% RTP — one of the most generous rates among crash games — and it stays at 98% across every difficulty level. Over time, that means 98% of all wagers flow back to players." },
      ...
      { q: "Is CluckRun fair and safe to play?", a: "Every round uses a provably fair system from InOut Games, so each result is generated randomly and can be independently verified. Neither the player nor the operator can predict or influence where the chicken stops." },
      { q: "Do I need to download an app or install anything?", a: "No. CluckRun runs straight in your browser on both desktop and mobile — no app store, no plugins, no installs. Just open the page and start playing." },
  ...
  ru: {
    ...
    demoDesc: "CluckRun — динамичная краш-игра от InOut Games с простой, но затягивающей механикой: вы управляете курицей, которая с каждым шагом через дорогу увеличивает вашу ставку. Доступны четыре уровня сложности, а RTP составляет щедрые 98%. Опробуйте всё это в живом демо ниже — без регистрации и без риска.",
    ...
    faqItems: [
      { q: "Какой минимальный депозит нужен, чтобы начать играть?", a: "Пополнить счёт можно на любую сумму начиная всего с $10 — не нужно вносить крупный банк, чтобы попробовать игру." },
      { q: "Какой RTP (процент возврата игроку) у CluckRun?", a: "CluckRun работает с высоким RTP 98% — одним из самых щедрых среди краш-игр — и он остаётся 98% на всех уровнях сложности. На дистанции это значит, что 98% всех ставок возвращается игрокам." },
      ...
      { q: "CluckRun — честная и безопасная игра?", a: "Каждый раунд использует систему provably fair от InOut Games: результат генерируется случайно и поддаётся независимой проверке. Ни игрок, ни оператор не могут предсказать или повлиять на то, где остановится курица." },
      { q: "Нужно ли скачивать приложение или что-то устанавливать?", a: "Нет. CluckRun запускается прямо в браузере на компьютере и на телефоне — без магазина приложений, плагинов и установки. Просто откройте страницу и играйте." },
  ...
  de: {
    ...
    faqItems: [
      { q: "Wie hoch ist die Mindesteinzahlung, um zu spielen?", a: "Du kannst dein Konto mit jedem Betrag ab nur 10 $ aufladen — du musst also kein großes Guthaben einsetzen, um loszulegen." },
      { q: "Wie hoch ist der RTP (Auszahlungsquote) von CluckRun?", a: "CluckRun läuft mit einem hohen RTP von 98 % — einer der großzügigsten Werte unter den Crash-Spielen — und bleibt in jeder Schwierigkeitsstufe bei 98 %. Langfristig fließen damit 98 % aller Einsätze an die Spieler zurück." },
      ...
      { q: "Ist CluckRun fair und sicher?", a: "Jede Runde nutzt ein nachweislich faires (Provably-Fair-)System von InOut Games, sodass jedes Ergebnis zufällig erzeugt und unabhängig überprüfbar ist. Weder Spieler noch Betreiber können vorhersagen oder beeinflussen, wo das Huhn stehen bleibt." },
      { q: "Muss ich eine App oder etwas anderes installieren?", a: "Nein. CluckRun läuft direkt im Browser — auf Desktop und Handy, ohne App-Store, Plugins oder Installation. Einfach die Seite öffnen und spielen." },
  ...
  fr: {
    ...
    demoDesc: "CluckRun est un jeu crash rythmé d'InOut Games aux mécaniques simples mais captivantes : vous dirigez une poule qui augmente votre mise à chaque pas sur la route. Choisissez parmi quatre niveaux de difficulté et profitez d'un généreux RTP de 98 %. Faites-en l'essai dans la démo en direct ci-dessous — sans inscription ni risque.",
    ...
    faqItems: [
      { q: "Quel est le dépôt minimum pour commencer à jouer ?", a: "Vous pouvez approvisionner votre compte avec n'importe quel montant à partir de seulement 10 $ — pas besoin d'engager une grosse somme pour vous lancer." },
      { q: "Quel est le RTP (taux de redistribution) de CluckRun ?", a: "CluckRun affiche un RTP élevé de 98 % — l'un des plus généreux parmi les jeux crash — et il reste à 98 % à tous les niveaux de difficulté. Sur la durée, 98 % des mises reviennent donc aux joueurs." },
      ...
      { q: "CluckRun est-il équitable et sûr ?", a: "Chaque manche utilise un système provably fair (équité prouvée) d'InOut Games : chaque résultat est généré aléatoirement et vérifiable indépendamment. Ni le joueur ni l'opérateur ne peuvent prédire ou influencer l'endroit où la poule s'arrête." },
      { q: "Dois-je télécharger une application ou installer quelque chose ?", a: "Non. CluckRun fonctionne directement dans votre navigateur, sur ordinateur comme sur mobile — sans store, sans plugin, sans installation. Ouvrez simplement la page et jouez." },
  ...
  es: {
    ...
    demoDesc: "CluckRun es un juego crash trepidante de InOut Games con mecánicas sencillas pero adictivas: controlas una gallina que aumenta tu apuesta con cada paso que da por la carretera. Elige entre cuatro niveles de dificultad y juega con un generoso RTP del 98 %. Pruébalo en la demo en vivo de abajo, sin registro y sin riesgo.",
    ...
    faqItems: [
      { q: "¿Cuál es el depósito mínimo para empezar a jugar?", a: "Puedes recargar tu cuenta con cualquier importe a partir de solo 10 $, así que no necesitas comprometer un gran saldo para empezar." },
      { q: "¿Cuál es el RTP (retorno al jugador) de CluckRun?", a: "CluckRun funciona con un alto RTP del 98 %, uno de los más generosos entre los juegos crash, y se mantiene en el 98 % en todos los niveles de dificultad. A largo plazo, eso significa que el 98 % de las apuestas vuelve a los jugadores." },
      ...
      { q: "¿Es CluckRun justo y seguro?", a: "Cada ronda usa un sistema provably fair (de equidad comprobable) de InOut Games, por lo que cada resultado se genera de forma aleatoria y se puede verificar de manera independiente. Ni el jugador ni el operador pueden predecir o influir en dónde se detiene la gallina." },
      { q: "¿Necesito descargar una app o instalar algo?", a: "No. CluckRun se ejecuta directamente en tu navegador, tanto en ordenador como en móvil: sin tiendas de apps, sin complementos y sin instalaciones. Solo abre la página y juega." },
  ...
  pt: {
    ...
    faqItems: [
      { q: "Qual é o depósito mínimo para começar a jogar?", a: "Você pode abastecer sua conta com qualquer valor a partir de apenas $10, então não precisa comprometer um saldo grande para começar." },
      { q: "Qual é o RTP (retorno ao jogador) do CluckRun?", a: "O CluckRun funciona com um RTP alto de 98% — um dos mais generosos entre os jogos crash — e permanece em 98% em todos os níveis de dificuldade. A longo prazo, isso significa que 98% das apostas voltam aos jogadores." },
      ...
      { q: "O CluckRun é justo e seguro?", a: "Cada rodada usa um sistema provably fair (de justiça comprovável) da InOut Games, então cada resultado é gerado aleatoriamente e pode ser verificado de forma independente. Nem o jogador nem o operador conseguem prever ou influenciar onde a galinha para." },
      { q: "Preciso baixar um aplicativo ou instalar algo?", a: "Não. O CluckRun roda direto no navegador, no computador e no celular — sem loja de aplicativos, sem plugins e sem instalações. É só abrir a página e jogar." },
